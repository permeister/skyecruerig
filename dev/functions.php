...
function wprig_fonts_url() {
	...
	/**
	 * Translator: If Open Sans doesnt support characters in your language, translate this to 'off'.
	 */
	$opensans = esc_html_x( 'on', 'Open Sans font: on or off', 'wprig' );

	/**
	 * Translator: If Pacifico does not support characters in your language, translate this to 'off'.
	 */
	$pacifico = esc_html_x( 'on', 'Pacifico font: on or off', 'wprig' );
	...
	if ( 'off' !== $opensans ) {
		$font_families[] = 'Open Sans:200,400,400i,700';
	}
	...
	if ( in_array( 'on', array( $raleway, $opensans, $pacifico ) ) ) {
		$query_args = array(
